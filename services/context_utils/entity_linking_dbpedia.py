...
def _sparql_entity_lookup(entity_name: str) -> list[dict]:
...
def dbpedia_el(nlq: str, ne_list: list) -> list:
    ...
    texts = [nlq] + ne_list
    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(_falcon_external, t) for t in texts]
    ...
    if not nel_list and ne_list:
        ...
        nel_list = _dbpedia_el_sparql(nlq, ne_list)
