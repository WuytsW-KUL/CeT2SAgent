...
def _falcon_external_wikidata(text: str) -> list:
...
def _wikidata_api_search(entity_name: str, lang: str = "en", similarity: int = 85) -> list:
...
def wikidata_el(nlq: str, ne_list: list) -> list:
    ...
    with ThreadPoolExecutor() as executor:
        falcon_results = list(executor.map(_falcon_external_wikidata, texts))
    ...
    if missing:
        with ThreadPoolExecutor() as executor:
            api_results = list(executor.map(_wikidata_api_search, missing))
